...
export interface LearningLog {
  ...
  ai_status?: AiEnhancementStatus;
  ai_job_id?: string | null;
  updated_at?: string | null;
}
export interface LearningRecordInput {
  kind: Exclude<LearningRecordKind, "observation" | "photo_activity" | "material_use">;
  title: string;
  occurred_at?: string | null;
  subject?: string | null;
  institution?: string | null;
...
export interface InfantObservationHints {
  source: string;
  effective_date: string;
  diagnostic: boolean;
  hints: ObservationHint[];
}
export interface BoardBookRecommendation {
...
export interface ConversationAnswer {
  session_id: string;
  thread_id: string;
  answer: { answer: string; source_ids: string[]; insufficient_evidence: boolean };
  turn_count: number;
}
...
export interface ResourceRecord extends ResourceCreateInput {
  id: string;
  created_at?: string;
  updated_at?: string;
}
...
export interface GeneratedMaterial {
  ...
  curriculum_targets?: CurriculumTarget[];
  parent_guide_markdown?: string;
  ai_status?: AiEnhancementStatus;
  ai_job_id?: string | null;
  created_at?: string;
  updated_at?: string;
...
export interface PhotoActivityRecord {
  id: string;
  child_id: string;
  ...
  suggested_experience_axes: ExperienceAxis[];
  suggested_interest: string | null;
  suggested_difficulty_note: string | null;
...
export interface EntityLinkGraphItem {
  link: EntityLink;
  entity: Record<string, unknown> | null;
}
export interface EntityBacklinks {
  incoming: EntityLinkGraphItem[];
  outgoing: EntityLinkGraphItem[];
}
...
export const listObservations = (childId: string) =>
  call<LearningLog[]>("list_observations", { childId });
export const createLearningRecord = (childId: string, input: LearningRecordInput) =>
  call<LearningLog>("create_learning_record", {
    childId,
    request: {
      ...input,
      ...
      difficulty_note: input.difficulty_note ?? null,
      next_activity: input.next_activity ?? null,
      tags: input.tags ?? [],
      experience_axes: input.experience_axes ?? [],
      shared_child_ids: input.shared_child_ids ?? [],
    },
  });
export const listLearningRecords = (childId: string) =>
  call<LearningLog[]>("list_learning_records", { childId });
export const createActivity = (childId: string, title: string, sourceRefs: string[] = []) =>
  call<ActivityPlan>("create_activity", { childId, title, sourceRefs });
export const listActivities = (childId: string) =>
  call<ActivityPlan[]>("list_activities", { childId });
export const transitionActivity = (
  activityId: string,
  status: ActivityStatus,
  parentNote?: string,
) => call<ActivityPlan>("transition_activity", { activityId, status, parentNote: parentNote ?? null });
export const listActivityObservations = (activityId: string) =>
  call<LearningLog[]>("list_activity_observations", { activityId });
export const searchChildContext = (childId: string, query: string) =>
  call<SearchResponse>("search_child_context", { childId, query });
export const createConversation = (childId: string) =>
  call<ConversationSession>("create_conversation", { childId });
export const listConversations = (childId: string) =>
  call<ConversationSession[]>("list_conversations", { childId });
export const appendConversationTurn = (sessionId: string, question: string) =>
  call<ConversationAnswer>("append_conversation_turn", { sessionId, question });
export const createResource = (request: ResourceCreateInput) =>
...
export const generateMaterial = (
  ...
  call<GeneratedMaterial>("generate_material_background", {
    childId,
    kind,
    topic,
    goal: goal ?? null,
...
export const listMaterials = (childId: string) =>
...
export const editMaterial = (
  ...
  note?: string | null,
) => call<GeneratedMaterial>("edit_material", { materialId, title, contentMarkdown, note: note ?? null });
export const recordMaterialResult = (materialId: string, input: MaterialResultInput) =>
  call<MaterialResultResponse>("record_material_result", {
    materialId,
    request: {
      ...input,
      learner_work: input.learner_work ?? null,
...
export const listMaterialResults = (materialId: string) =>
  call<MaterialUseHistoryItem[]>("list_material_results", { materialId });
export const getGrowthMap = (childId: string) =>
  call<GrowthMap>("get_growth_map", { childId });
export const getInfantActivities = (childId: string) =>
  call<InfantActivitySuggestions>("get_infant_activities", { childId });
export const getInfantObservationHints = (childId: string) =>
  call<InfantObservationHints>("get_infant_observation_hints", { childId });
export const getBoardBookRecommendations = (childId: string) =>
  call<BoardBookRecommendations>("get_board_book_recommendations", { childId });
...
export const listPhotoRecords = (childId: string) =>
  call<PhotoActivityRecord[]>("list_photo_records", { childId });
export const commitPhotoRecord = (recordId: string, observation?: string) =>
...
export const restoreBackup = (archiveName: string) =>
  call<BackupRestoreResult>("restore_backup", { archiveName });
export const exportBackup = (archiveName: string) =>
  call<string | null>("export_backup", { archiveName });
export const importBackup = () =>
  call<(BackupRestoreResult & { safety_backup: string; imported_archive: string }) | null>(
    "import_backup",
  );
