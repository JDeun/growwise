...
export function MaterialWorkspace(props: MaterialWorkspaceProps) {
  ...
  return <section className="material-workspace" aria-labelledby="materials-title">
    ...
    <form className="material-composer" onSubmit={onGenerate}>
      <fieldset className="material-kind-picker"><legend>무엇을 만들까요?</legend><div className="material-kind-options">{catalog.map((item) => <label key={item.kind} className={`material-kind-option${materialKind === item.kind ? " selected" : ""}`}><input type="radio" name="material-kind" value={item.kind} checked={materialKind === item.kind} onChange={() => onKindChange(item.kind)} disabled={busy}/><span><strong>{item.label}</strong><small>{item.description}</small></span></label>)}</div></fieldset>
      <label className="material-topic-field"><span>주제</span><input value={topic} onChange={(e) => onTopicChange(e.target.value)} maxLength={200} placeholder={selectedCatalogItem.placeholder} disabled={busy}/></label>
      ...
      <button className="primary-button" type="submit" disabled={busy || !topic.trim()}>{busy ? "처리 중…" : `${selectedCatalogItem.label} 만들기`}</button>{error && <p className="form-error" role="alert">{error}</p>}
    </form>
