...
function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [health, runtime, storedChildren, storedBackups] = await Promise.all([
        getHealth(),
        getCoreRuntimeStatus(),
        listChildren(),
        listBackups(),
  ...
  async function handleReviseMaterial(materialId: string) {
    ...
    try {
      ...
      setMaterialError(error instanceof Error ? error.message : "수정본 생성에 실패했습니다.");
    } finally {
      setMaterialBusy(false);
    }
  }
