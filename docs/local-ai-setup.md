...
별도 설정 없이 GrowWise의 현재 기본 모델 ID를 그대로 사용하려면 Ollama 설치 후 다음 세 모델을 받습니다.
...
위 명령에 세 모델이 보이면 기본 준비가 끝난 것입니다.
...
현재 pre-1.0 빌드에서는 모델 ID 변경이 환경변수 기반입니다.
...
2. `ollama list`에서 설정한 모델 이름이 정확히 존재하는지 확인합니다.
3. 모델이 너무 크다면 더 작은 모델로 바꿉니다.
4. 그래도 실패해도 GrowWise의 로컬 기록 기능은 그대로 사용할 수 있습니다.
