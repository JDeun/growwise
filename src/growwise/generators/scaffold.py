...
class ScaffoldGuard:
    ...
    _INJECTION_PATTERNS = (
        ...
        re.compile(r"(?:reveal|print|show|repeat)\s+(?:the\s+)?(?:system|developer)\s+(?:prompt|message|instructions?)", re.IGNORECASE),
    ...
    _REVIEW_BYPASS_PATTERNS = (
        re.compile(r"(?:skip|bypass|disable)\s+(?:the\s+)?(?:parent\s+)?review", re.IGNORECASE),
    ...
    def check(self, *, kind: MaterialKind, title: str, content: str) -> ScaffoldCheck:
        text = f"{title}\n{content}".casefold()
        violations: list[str] = []
        if kind in self._SCAFFOLD_KINDS and any(pattern.search(text) for pattern in self._DIRECT_ANSWER_PATTERNS):
